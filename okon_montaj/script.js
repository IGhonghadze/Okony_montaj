...
function renderCart() {
    ...
    let breakdown = { sill: 0, ebb: 0, prof: 0, net: 0, sand: 0 };
    ...
    ITEMS.forEach((it, i) => {
        ...
        if(it.linM) {
            ...
            sumLinM += addL;
            if(it.type) breakdown[it.type] += addL;
        }
        if(it.sqM) {
    ...
    document.getElementById('sum-lin-breakdown').innerText = `(Подоконники: ${breakdown.sill.toFixed(2)} п.м. | Отливы: ${breakdown.ebb.toFixed(2)} п.м. | Профиля: ${breakdown.prof.toFixed(2)} п.м.)`;
